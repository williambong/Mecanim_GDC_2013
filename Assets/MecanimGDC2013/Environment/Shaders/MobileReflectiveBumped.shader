Shader "Mobile/ReflectiveBumped" {
...
SubShader {
	...
	Pass {
		...
		fixed4 frag (v2f_full i) : COLOR0 
		{
			...
			half4 reflection = texCUBE (_Cube, reflectVector);
			
			tex += reflection * saturate(tex.a - _OneMinusReflectivity);
												
			#ifdef LIGHTMAP_ON
				half3 lm =  DecodeLightmap (tex2D(unity_Lightmap, i.uv.zw));
				tex.rgb *= lm;
			#else
				tex.rgb *= 0.75;
			#endif	
